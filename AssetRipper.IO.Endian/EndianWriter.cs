...
namespace AssetRipper.IO.Endian
{
	public class EndianWriter : BinaryWriter
	{
		...
		public EndianWriter(Stream stream, bool isLittleEndian) : base(stream) 
		{
			isBigEndian = !isLittleEndian;
		}

		protected EndianWriter(Stream stream, bool isLittleEndian, Encoding encoding) : base(stream, encoding)
		{
			isBigEndian = !isLittleEndian;
		}

		protected EndianWriter(Stream stream, bool isLittleEndian, Encoding encoding, bool leaveOpen) : base(stream, encoding, leaveOpen)
		{
			isBigEndian = !isLittleEndian;
		}
		...
		public void WriteArray(bool[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(bool[] buffer, int index, int count, bool align = false)
		{
		...
		public void WriteArray(char[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(char[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(byte[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(byte[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
			Write(buffer, index, count);
		...
		public void WriteArray(short[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(short[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(ushort[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(ushort[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(int[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(int[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(uint[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(uint[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(long[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(long[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(ulong[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(ulong[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(float[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(float[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(double[] buffer, bool align = false)
		{
			WriteArray(buffer, 0, buffer.Length, align);
		}

		public void WriteArray(double[] buffer, int index, int count, bool align = false)
		{
			FillInnerBuffer(count);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteArray(string[] buffer, bool align = false)
		{
			FillInnerBuffer(buffer.Length);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteEndianArray<T>(T[] buffer, bool align = false) where T : IEndianWritable
		{
			FillInnerBuffer(buffer.Length);
			Write(m_buffer, 0, sizeof(int));
		...
		public void WriteEndianArray<T>(T[][] buffer, bool align = false) where T : IEndianWritable
		{
			...
			{
				WriteEndianArray(buffer[i], align);
			}
			if (align)
			{
				AlignStream();
